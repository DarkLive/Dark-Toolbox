        ...
        private void seppo_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.Startup) + "\\DarkStartup.bat"))
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    ...
                    {
                        setactions.Items.Add("Start Software: " + line.Replace("start \"\" ", ""));
                    }
                    else if (line.StartsWith("taskkill"))
                    {
                        setactions.Items.Add("Stop Software: " + line.Replace("taskkill /f /im ", ""));
                    }
                    else if (line.StartsWith("net start"))
                    {
                        setactions.Items.Add("Start Service: " + line.Replace("net start ", ""));
                    }
                    else if (line.StartsWith("net stop"))
                    {
                        setactions.Items.Add("Stop Service: " + line.Replace("net stop ", ""));
                    }
                    else if (line.StartsWith("sleep"))
                    {
                        setactions.Items.Add("Wait " + line.Replace("sleep ", "")+" Second");
                    }
                    else
                    {
                        setactions.Items.Add("NOT SUPPORTED ACTION");
                    }
                }
                file.Close();
        ...
        private void Add_Click(object sender, RoutedEventArgs e)
        {
            using (StreamWriter writer = new StreamWriter(Environment.GetFolderPath(Environment.SpecialFolder.Startup) + "\\DarkStartup.bat",true))
            {
                ...
                else if (actionlist.SelectionBoxItem.ToString() == "Wait \"x\" Seconds")
                {
                    ...
                    {
                        MessageBox.Show("Please check the number you entered.");
                    }
                }
                else
                {
